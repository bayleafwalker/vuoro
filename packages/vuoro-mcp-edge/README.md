...
- **Inbound auth is the gateway assertion and nothing else.** The Ed25519 JWT
  ...
  environment, repository binding and request-id correlation. A missing or
...
|---|---|---|---|---|
| `list_ready_work` `{limit?: 1..50 = 50}` | read | `work:read` | `work.public.list-v1` | items with `blocked == false`, owner order, each exactly `work_id, title, priority, status, blocked, updated_at` |
| `describe_work` `{work_id: integer >= 1}` | read | `work:read` | `work.public.item-v1` | one item in any status, exactly the six list fields plus `created_at, resolution, blocked_by` |
...
the record does not reach the client. Rejections (`postgres-runtime-unavailable`,
...
empty list, so an empty `items` means there is no ready work.

## Protocol

One stateless endpoint, `POST /mcp`, answering plain JSON. `GET` and `DELETE`
return 405. There is no SSE and no `Mcp-Session-Id`. Both protocol eras work:
`initialize` (echoing 2024-11-05, 2025-03-26 or 2025-06-18, otherwise
