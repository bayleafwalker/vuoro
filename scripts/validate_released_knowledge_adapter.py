...
def _knowledge_pins(manifest_path: Path) -> tuple[dict, dict]:
    ...
    if descriptor["dependency_lock_ids"] != ["vuoro-adapter-kit"]:
        raise SystemExit("knowledge descriptor must identify exactly the adapter-kit dependency")
    ...
    if dependency.get("lock_kind") != "shared-dependency":
        raise SystemExit("knowledge dependency must be a shared dependency")
...
def main(argv: list[str] | None = None) -> int:
    ...
    adapter, dependency = _knowledge_pins(manifest)
